...
public class Player implements Entity {
	...
	public Shape getShape(int ticks) {
		return myShape.transform(Transform.createTranslateTransform(
				oldPosition.x + (velocity.x * ticks), oldPosition.y
						+ (velocity.y * ticks)));
	}
	...
	public Shape handleCollisions(int ticks, List<Collision> collisions) {
		...
		if ((them.getShape(earliest) instanceof Rectangle)) {
			newPosition = oldPosition.add(velocity
					.scale((float) (earliest - (1.0 / 1000))));
			// If I'm overlapping their xcoord
			if (me.getShape(earliest + 1).getMaxX() > them.getShape(
					earliest + 1).getMinX()) {
				velocity.x = 0;
			} else if (me.getShape(earliest + 1).getMinX() < them.getShape(
					earliest + 1).getMaxX()) {
				velocity.x = 0;
			}
			// If I'm overlapping their ycoord
			else if (me.getShape(earliest + 1).getMinY() < them.getShape(
					earliest + 1).getMaxY()) {
				velocity.y = 0;
			} else if (me.getShape(earliest + 1).getMinY() < them.getShape(
					earliest + 1).getMaxY()) {
